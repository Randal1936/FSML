...
#
# os.chdir('E:/ANo.3/base/3-5分词筛选样本')
...
def titles(Data, rules = None):
    ...
    if rules is None:
        rules = infos
    data = Data.copy()
    result = cj.infos_docs_freq(rules, data)
    dtm = result['DTM']
    dfc = result['DFC']
    ...
    result = {'DTM': dtm,  # 正文检索获得的 Doc Term Matrix
              'DFC': dfc,  # 正文检索获得的 Doc Frequency Context
              'DTM_final': dtm_final}  # 标题个数和标题种类数
